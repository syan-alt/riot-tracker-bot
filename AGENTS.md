...
- **Match-report tests above the API** use `src/fixtures/production-matches.json` via `pnpm fixtures:check` and `pnpm admin report-mock`.
- **Exercise reporting without a live match:** with `DEV_MODE=true` the bot registers `/dev_report <game>` (posts a scoreboard embed from a production fixture), `/dev_signup`, and `/dev_clear`. These are Discord slash commands invoked from a Discord client in the test server.
