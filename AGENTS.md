...
- **Secrets are ambient env vars, not a `.env` file.** Cursor Cloud receives only a testing bot token, the Riot/Henrik API keys, a designated `VERIFY_RIOT_ID`, the testing Discord destination, and `DEV_MODE=true`. Never give the `syan-alt/riot-tracker-bot` environment a production Discord token or destination, Railway credential, or production database. Run the bot with `pnpm start` (reads ambient env). `pnpm dev` fails here because it passes `--env-file=.env` and no `.env` exists.
