...
- **Secrets are ambient env vars, not a `.env` file.** Cursor Cloud receives only the `riot-tracker-dev` bot token, the Riot/Henrik API keys, a designated `VERIFY_RIOT_ID`, the riot-tracker-testing channel values, and `DEV_MODE=true`. Never give the `syan-alt/riot-tracker-bot` environment a production Discord token, production channel id, Railway credential, or production database. Run the bot with `pnpm start` (reads ambient env). `pnpm dev` fails here because it passes `--env-file=.env` and no `.env` exists.
...
- **Cloud agent verification (no Discord/Gmail login):** run `pnpm verify`. It starts the live bot, drives `pnpm admin` against an isolated sqlite file, and writes evidence to `/opt/cursor/artifacts/verify-<run-id>/`. `VERIFY_RIOT_ID` supplies the designated test account; the verifier never reads production Railway or a production database. See `.cursor/skills/verify-riot-tracker-bot/SKILL.md`.
- **Verify and mock reports post only to riot-tracker-testing.** Cloud agents run the bot locally (`pnpm start` / `pnpm verify`). The allowlisted guild is `1523432684691525802`; the allowlisted `#riot-tracker-dev` channel is `1523432733785722940`. `pnpm verify` ignores ambient `NOTIFICATION_CHANNEL_ID` and uses `VERIFY_NOTIFICATION_CHANNEL_ID`, `TESTING_NOTIFICATION_CHANNEL_ID`, or the channel from `DISCORD_TEST_CHANNEL_URL`. If those testing values are unset or do not match the allowlist, verify skips Discord send. `report-mock` and `DEV_MODE` also fail closed outside the allowlist.
